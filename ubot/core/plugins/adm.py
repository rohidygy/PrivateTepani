...
async def cek_allgc(client, query):
    chats = []
    chat_types = {
...
async def admin_bannen(client, message):
    if message.command[0] == "kick":
        ...
        if user_id == (await client.get_me()).id:
            return await message.reply_text("Tidak bisa menendang diri sendiri.")
        if user_id == OWNER_ID:
            return await message.reply_text("Dia owner bot anda.")
        if user_id in (await list_admins(message)):
            return await message.reply_text("Tidak bisa menendang admin.")
        ...
        if not user_id:
            return await message.reply_text("Tidak dapat menemukan pengguna.")
        if user_id == (await client.get_me()).id:
            return await message.reply_text("Tidak bisa memblokir diri sendiri.")
        if user_id == OWNER_ID:
            return await message.reply_text("Dia adalah Owner Bot anda.")
        if user_id in (await list_admins(message)):
            return await message.reply_text("Tidak dapat memblokir sesama admin.")
        try:
            mention = (await client.get_users(user_id)).mention
        except IndexError:
            mention = (
        ...
        if not user_id:
            return await message.reply_text("Tidak dapat menemukan pengguna.")
        if user_id == (await client.get_me()).id:
            return await message.reply_text("Tidak dapat membisukan diri sendiri.")
        if user_id == OWNER_ID:
            return await message.reply_text("Dia Owner Bot Anda.")
        if user_id in (await list_admins(message)):
            return await message.reply_text("Tidak bisa membisukan sesama admin.")
        mention = (await client.get_users(user_id)).mention
        await message.reply_to_message.delete()
